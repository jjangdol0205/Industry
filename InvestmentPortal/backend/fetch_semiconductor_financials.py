# -*- coding: utf-8 -*-
"""
반도체 산업 기업 재무제표 + 프로파일 수집 스크립트
industry_id=9 기업 대상 yfinance 데이터 수집
"""
import sqlite3
...
def main():
    ...
    cur.execute("SELECT id, name, ticker FROM companies WHERE industry_id=9 ORDER BY display_order, id")
    companies = cur.fetchall()

    if not companies:
        print("⚠️  반도체 기업이 없습니다. insert_semiconductor.py를 먼저 실행하세요.")
        conn.close()
        return

    print(f"=== 반도체 산업 {len(companies)}개 기업 재무데이터 수집 ===")
    print("="*65)
