# -*- coding: utf-8 -*-
"""
조선 산업 기업 재무제표 + 프로파일 수집 스크립트
industry_id=12 기업 대상 yfinance 데이터 수집
"""
import sqlite3
...
def main():
    ...
    cur.execute("SELECT id, name, ticker FROM companies WHERE industry_id=12 ORDER BY display_order, id")
    companies = cur.fetchall()

    if not companies:
        print("⚠️  조선 기업이 없습니다. insert_shipbuilding.py를 먼저 실행하세요.")
        conn.close()
        return

    print(f"=== 조선 산업 {len(companies)}개 기업 재무데이터 수집 ===")
    print("="*65)
    ...
    print("\n" + "="*65)
    print("=== 수집 결과 ===")
    print(f'{"ID":<4} {"Company":<30} {"Ticker":<12} {"Ann":<5} {"Qtr":<5} {"Records"}')
    print("-"*65)
    ok = fail = 0
    for r in results:
        status = "[OK]  " if r[3] > 0 else "[WARN]"
        print(f'{status} {r[0]:<4} {r[1]:<30} {r[2]:<12} {r[3]:<5} {r[4]:<5} {r[5]}')
        if r[3] > 0: ok += 1
        else: fail += 1
    print(f"\nOK={ok}, NoData={fail}")
