...
# 형식: (이름, 티커, 포트폴리오 티어, 산업 ID, 역할설명, 원칙충족이유)
# 산업 ID 매핑: 1=AI반도체, 2=로봇, 3=우주, 4=전력, 5=AI SW, 6=배터리, 8=온디바이스AI, 9=반도체, 10=에너지
# 신규 산업 ID 17을 "나스닥/S&P500 독점 우량주"로 생성
...
NEW_STOCKS = [
    ...
    },
    {
        ...
        "principle_reason": "제2원칙: 멀티클라우드 데이터 독점 플랫폼. 제3원칙: NRR 128%+ 고성장. Satellite 편입.",
    },
    {
        "name": "Eli Lilly",
        "ticker": "LLY",
        "portfolio_tier": "Satellite",
        "industry_id": 5,  # 편의상 AI SW 산업 사용
        "role_description": "GLP-1 비만/당뇨 치료제(젭바운드/마운자로) 글로벌 1위. 신약 특허 독점 2030년대까지.",
        "future_growth": "GLP-1 글로벌 시장 2030년 $150B 성장 선점, 주사 편의성 개선 경구형 GLP-1 파이프라인",
    ...
    },
    {
        ...
        "industry_id": 5,
        "role_description": "미국 최대 건강보험/의료 플랫폼(OptumHealth+Optum Rx). 의료 데이터 독점 플랫폼. 가입자 5500만+ 락인.",
    ...
    },
    {
        ...
        "portfolio_tier": "Watchlist",
        "industry_id": 5,
        "role_description": "상업용 부동산 데이터 플랫폼 1위 독점. Apartments.com 주거 임대 정보 1위. 데이터 네트워크 효과.",
        "future_growth": "주거 부동산 데이터 사업 확장, AI 부동산 분석 플랫폼, 해외 진출(유럽 CoStar)",
        "principle_reason": "제2원칙: 상업 부동산 데이터 독점. 제3원칙: 성장성 +10%+. Watchlist (가격 조정 대기). 편입.",
    },
    {
    ...
    # ── 추가 S&P500 우량 독점주 ──
    {
        ...
        "portfolio_tier": "Satellite",
        "industry_id": 9,
        "role_description": "AI 데이터센터 커스텀 ASIC 고성장(Amazon/Microsoft 설계), 광전자 집적회로(DSP) 1위. 5G 인프라 반도체.",
        "future_growth": "AI ASIC 매출 YoY +100%+ 성장 구간, 광전자 IC 병목 해소 불가, 데이터센터 인프라 확산",
    ...
    },
    {
        ...
        "portfolio_tier": "Satellite",
        "industry_id": 5,
        "role_description": "클라우드 보안 플랫폼 통합(SASE+XDR+SOAR) 1위. Cortex AI 위협 대응 자동화. 플랫폼화 전략.",
...
def update_universe():
            ...
            if tier_rank.get(s['portfolio_tier'],5) < tier_rank.get(old_tier,5):
                ...
                try:
                    ...
                    if not hist.empty:
                        ...
                        mdd_pct = ((current_price - high_52w) / high_52w) * 100
                        mdd_pct = round(mdd_pct, 2)
                        
                        prof = cur.execute("SELECT id FROM company_profiles WHERE company_id=?", (comp_id,)).fetchone()
                        if prof:
                            cur.execute("""
                                UPDATE company_profiles SET current_price=?, high_52w=?, mdd_pct=?, last_updated=?
                                WHERE company_id=?
                            """, (current_price, high_52w, mdd_pct, now, comp_id))
                        else:
                            buy_signal = 'WAIT'
                        ...
                        price_updated += 1
                        print(f"    가격 업데이트: ${current_price:.2f} (52wH: ${high_52w:.2f}, MDD: {mdd_pct:.1f}%)")
        ...
        try:
            ...
            if not hist.empty:
                ...
                if prof:
                    ...
                    """, (comp_id, current_price, high_52w, mdd_pct, buy_signal, now))
                price_updated += 1
                print(f"    가격: ${current_price:.2f} (52wH: ${high_52w:.2f}, MDD: {mdd_pct:.1f}%, {buy_signal})")
            else:
                print(f"    ⚠️ 가격 데이터 없음: {ticker}")
        except Exception as e:
    ...
    print(f"\n✅ 완료: {added}개 신규 추가, {updated}개 티어 업그레이드, {price_updated}개 가격 업데이트")
    return added, updated, price_updated

if __name__ == '__main__':
    print("🚀 나스닥/S&P500 독점 우량주 유니버스 추가 시작...")
    update_universe()
