# -*- coding: utf-8 -*-
"""
전력 인프라 산업 10개 기업 재무제표 + 프로파일 수집 스크립트
industry_id=6 기업 대상 yfinance 데이터 수집
"""
import sqlite3
...
def fetch_data(ticker_str):
    ...
    def process(date_col, ptype, inc, bs, cf):
        ...
        capex = try_keys(cf, ['Capital Expenditure', 'Capital Expenditures',
                               'Purchase Of Property Plant And Equipment'], date_col)
...
def main():
    ...
    # 전력 인프라 산업(industry_id=6) 기업만 대상
    cur.execute("SELECT id, name, ticker FROM companies WHERE industry_id=6 ORDER BY display_order, id")
    companies = cur.fetchall()

    if not companies:
        print("⚠️  전력 인프라 기업이 없습니다. init_power_infra_industry.py를 먼저 실행하세요.")
        conn.close()
        return

    print(f"=== 전력 인프라 산업 {len(companies)}개 기업 재무데이터 수집 ===")
    print("="*65)
    ...
    print("\n" + "="*65)
    print("=== 수집 결과 ===")
    print(f'{"ID":<4} {"Company":<30} {"Ticker":<6} {"Ann":<5} {"Qtr":<5} {"Records"}')
    print("-"*58)
    ok = fail = 0
    for r in results:
        status = "[OK]  " if r[3] > 0 else "[WARN]"
        print(f'{status} {r[0]:<4} {r[1]:<30} {r[2]:<6} {r[3]:<5} {r[4]:<5} {r[5]}')
        if r[3] > 0: ok += 1
        else: fail += 1
    print(f"\nOK={ok}, NoData={fail}")
