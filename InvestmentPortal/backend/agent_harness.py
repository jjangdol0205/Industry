...
## Executive Summary
현재 자율주행·로봇·우주·코인·에너지 5개 산업 모두 AI 인프라 확충 단계에서 핵심 컴퓨팅 자원 및 전력 부족이라는 공통 병목에 직면해 있습니다. SMR(소형모듈원전) 및 원가 보상형 PPA 계약이 전력 병목의 핵심 해결책으로 부상하고 있으며, 빅테크의 무조건부 인수 계약(Take-or-Pay)이 기업 가치 재평가(Re-rating)의 기폭제 역할을 하고 있습니다.
...
| 에너지 | AI 전력 수요 폭증 및 SMR 본격화 | OVERWEIGHT | 무조건부 인수 계약(Take-or-Pay PPA) |

## 크로스인더스트리 병목 분석
5개 산업의 공통 병목은 **'물리적 AI 구동을 위한 고성능 컴퓨팅 및 CFE 전력 인프라'**입니다. AI 연산의 물리적 확장 한계는 송배전망 부족과 냉각/전기 공급 병목으로 전이되었습니다. 이를 해결하기 위해 데이터센터 옆에 SMR을 건설하여 직접 전력을 꽂는 Behind the Meter(BTM) 방식과 가스터빈 발전이 핵심 인프라로 자리 잡고 있습니다.

## Top 3 Pick & 포트폴리오 배분
### 1위: NVIDIA (NVDA) — 배분 비중: 30%
자율주행·로봇·AI 전 산업의 두뇌 독점. CUDA 생태계 전환 비용 無. 데이터센터 매출 YoY +100%+.

### 2위: Centrus Energy (LEU) — 배분 비중: 25%
SMR 고효율 핵연료 'HALEU'의 미국 내 유일한 상업 농축 독점 공급사. 러시아 의존 탈피 정책 수혜 및 수주잔고 기하급수 성장의 수혜주.

### 3위: Coinbase (COIN) — 배분 비중: 20%
미국 최대 규제 준수 거래소 + 기관 수탁(Custody) 독점. ETF 수탁 파트너 및 규제 완화 수혜로 강력한 캐시카우 등극.

## 투자 시나리오
| 시나리오 | 확률 | 수익률 전망 | 트리거 |
|---------|------|-----------|-------|
| Bull Case | 35% | +40~60% | 금리 인하 + SMR 인허가 조기 획득 + AI 전력 수요 폭증 |
| Base Case | 45% | +15~25% | 현 성장 지속 + 점진적 규제 명확화 + PPA 체결 가속 |
| Bear Case | 20% | -15~25% | 경기침체 + 송배전망 규제 강화 + 유동성 위기 |

## 모니터링 체크리스트
1. **SMR 기업의 미국 NRC 표준설계인가(SDA) 승인 여부** — 6개월 내 일정 모니터링
2. **빅테크와 원전 운영사의 장기 전력구매계약(PPA) 발표** — 뉴스 추적
3. **BTC 가격 & ETF 자금 유입** — 주간 모니터링
4. **NVDA 분기 실적 및 HALEU 보조금 규제 통과 여부**
5. **휴머노이드 로봇 양산 일정 및 테슬라 옵티머스 배치**"""
