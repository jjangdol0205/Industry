# -*- coding: utf-8 -*-
"""
코인 산업 14개 기업 재무제표 + 프로파일 수집 스크립트
"""
import sqlite3
...
def main():
    ...
    for cid, name, ticker in companies:
        ...
        # BKKT, CIFR, BRPHF 등 소형주는 데이터가 적을 수 있음
        try:
    ...
    print("\n" + "="*65)
    print("=== 수집 결과 ===")
    print(f'{"ID":<4} {"Company":<30} {"Ticker":<8} {"Ann":<5} {"Qtr":<5} {"Records"}')
    print("-"*58)
    ok = fail = 0
    for r in results:
        status = "[OK]  " if r[3] > 0 else "[WARN]"
        print(f'{status} {r[0]:<4} {r[1]:<30} {r[2]:<8} {r[3]:<5} {r[4]:<5} {r[5]}')
        if r[3] > 0: ok += 1
        else: fail += 1
    print(f"\nOK={ok}, NoData={fail}")
